...
module Algebra.Ring.Polynomial.QuotientOld ( Quotient(), QIdeal(), reifyQuotient, modIdeal
...
multUnamb :: (Eq r, Reifies ideal (QIdeal r ord n), IsMonomialOrder ord, IsPolynomial r n, Field r)
          => Quotient r ord n ideal -> Quotient r ord n ideal
          -> Quotient r ord n ideal
multUnamb a b = unamb (a * b) (multUnamb a b)

multWithTable :: (Eq r, Reifies ideal (QIdeal r ord n), IsMonomialOrder ord, IsPolynomial r n, Field r)
              => Quotient r ord n ideal -> Quotient r ord n ideal
              -> Quotient r ord n ideal
multWithTable f g =
  let qid = reflect f
...
buildMultTable :: (Eq r, IsMonomialOrder ord, IsPolynomial r n, Field r)
...
stdMonoms :: forall r n ord. (Eq r, IsMonomialOrder ord, IsPolynomial r n, Field r) => [OrderedPolynomial r ord n] -> Maybe [Monomial n]
stdMonoms basis = do
  let lms = map leadingTerm basis
      ...
      mexp (val, test) = [ V.foldr (+) 0 $ V.zipWith (*) val lm0
                         | (c, lm0) <- lms, c /= zero
                         , let a = V.foldr (+) 0 $ V.zipWith (*) lm0 test, isZero a
                         ]
...
genQuotVars' :: forall ord n ideal r. ( Eq r, Reifies ideal (QIdeal r ord (S n))
...
genQuotVars :: forall ord n ideal r. (Eq r, IsMonomialOrder ord, Reifies ideal (QIdeal r ord (S n))
...
modIdeal :: forall ord r n ideal. ( Eq r, IsMonomialOrder ord, Reifies ideal (QIdeal r ord n)
                                  , IsPolynomial r n, Field r)
...
modIdeal' :: (Eq r, IsMonomialOrder ord, Reifies ideal (QIdeal r ord n), IsPolynomial r n, Field r)
...
instance (Eq r, IsMonomialOrder ord, Reifies ideal (QIdeal r ord n), IsPolynomial r n, Field r) => Multiplicative (Quotient r ord n ideal) where
  f * g = modIdeal $ quotRepr_ f * quotRepr_ g
instance (Eq r, IsMonomialOrder ord, Reifies ideal (QIdeal r ord n), IsPolynomial r n, Field r) => Semiring (Quotient r ord n ideal) where
instance (Eq r, IsMonomialOrder ord, Reifies ideal (QIdeal r ord n), IsPolynomial r n, Field r) => Unital (Quotient r ord n ideal) where
  one   = modIdeal one
instance (Eq r, IsMonomialOrder ord, Reifies ideal (QIdeal r ord n), IsPolynomial r n, Field r) => Rig (Quotient r ord n ideal) where
instance (Eq r, IsMonomialOrder ord, Reifies ideal (QIdeal r ord n), IsPolynomial r n, Field r) => Ring (Quotient r ord n ideal) where
instance (Eq r, IsMonomialOrder ord, Reifies ideal (QIdeal r ord n), IsPolynomial r n, Field r) => LeftModule (Scalar r) (Quotient r ord n ideal) where
  r .* f = Quotient $ r .* quotRepr_ f
instance (Eq r, IsMonomialOrder ord, Reifies ideal (QIdeal r ord n), IsPolynomial r n, Field r) => RightModule (Scalar r) (Quotient r ord n ideal) where
  f *. r = Quotient $ quotRepr_ f *. r

instance (Eq r, IsMonomialOrder ord, Num r, Reifies ideal (QIdeal r ord n), IsPolynomial r n, Field r) => Num (Quotient r ord n ideal) where
  (+) = (NA.+)
  (*) = (NA.*)
...
isZeroDimensional :: (Eq r, Division r, IsPolynomial r n, IsMonomialOrder ord) => [OrderedPolynomial r ord n] -> Bool
